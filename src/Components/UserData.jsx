...
const UserData = ({ user, index, onStatusUpdate }) => {
  ...
  const getLocalDateFromId = (id) => {
    ...
    try {
      ...
      return new Date(createdDate.getTime() - offset)
        .toISOString()
        .split("T")[0];
    } catch (e) {
      return "N/A";
  ...
  const generatedUserId = userId
    ? `USR-${userId.substring(0, 10).toUpperCase()}`
    : `USR-2026${index}`;
    ...
    <tr className="hover:bg-neutral-950/60 transition-all border-b border-neutral-900 text-xs text-neutral-400 group">
        ...
        <div className="space-y-1">
          ...
          >
            ...
            <span className="select-all">
              {copiedField === "phone" ? "Copied Phone!" : user.phone || "N/A"}
            </span>
          </div>
      ...
      <td className="p-4 whitespace-nowrap">
        <div className="space-y-1">
          ...
          <p
            className="text-neutral-500 max-w-[180px] truncate text-[11px]"
            title={user.thana}
          >
      ...
      >
        <div className="flex flex-col items-center justify-center gap-1.5">
          <span
            className={`px-2 py-0.5 rounded border text-[9px] font-bold font-mono tracking-wider uppercase ${
              isPending
            ...
            <button
              onClick={() =>
                onStatusUpdate(
                  userId,
                  "PROCESSING",
                  "আবেদনটি প্রসেসিং এ পাঠানো হয়েছে!",
                )
              }
            ...
            <button
              onClick={() =>
                onStatusUpdate(
                  userId,
                  "COMPLETED",
                  "আবেদনটি সফলভাবে সম্পন্ন হয়েছে!",
                )
              }
          ...
          {!isCancelled ? (
            <button
              onClick={() => {
                if (
                  window.confirm(
                    "আপনি কি নিশ্চিত যে এই আবেদনটি বাতিল (Cancel) করতে চান?",
                  )
                ) {
                  onStatusUpdate(
                    userId,
